...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("##### Leader(s%d) has been disconnected by tester", leader1)
	cfg.checkOneLeader()
	DPrintf("##### Checked one leader")
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	DPrintf("##### Leader(s%d) has been reconnected by tester", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf("##### Leader((s%d)) has been disconnected by tester again", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("##### Another guy(s%d) disconnected by tester, now only 1 guy left", (leader2 + 1) % servers)

	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	DPrintf("##### Checked no leader")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("##### Some guy(s%d) reconnected by tester", (leader2 + 1) % servers)
	...
	cfg.connect(leader2)
	DPrintf("##### Another guy(s%d) reconnected by tester", leader2)
	cfg.checkOneLeader()
	DPrintf("##### Checked one leader")
	DPrintf("##### The End...")
	cfg.end()
